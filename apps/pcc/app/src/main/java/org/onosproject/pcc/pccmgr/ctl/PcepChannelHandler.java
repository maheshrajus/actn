...
class PcepChannelHandler extends IdleStateAwareChannelHandler {
    ...
    PcepChannelHandler(Controller controller) {
        ...
        duplicatePceIdFound = Boolean.FALSE;
        pcepPacketStats = new PcepPacketStatsImpl();
        pcepConfig = PcepConfig.getInstance();
    }
    ...
    enum ChannelState {
        ...
        OPENSENT(false) {
            ...
            void processPcepMessage(PcepChannelHandler h, PcepMessage m) throws IOException, PcepParseException {
                ...
                if (m.getType() != PcepType.OPEN) {
                    ...
                    h.processUnknownMsg();
                    log.info("message is not OPEN message");
                } else {
                    ...
                    PcepOpenMsg pOpenmsg = (PcepOpenMsg) m;
                        ...
                        log.info("Sending handshake Keepalive message");
                        ...
                        if (h.thispceId == null) {
                            ...
                            h.thispceId = PceId.pceId(IpAddress.valueOf(inetAddress.getAddress()));
                        }

                        h.sendKeepAliveMessage();
                        h.pcepPacketStats.addOutPacket();
                        h.setState(OPENCONFIRM);
                        h.pcepConfig.setPeerConnState(h.peerAddr, PcepPeerCfg.State.OPENCONFIRM);
                }
            }
        },
        /**
        ...
        OPENCONFIRM(false) {
            ...
            void processPcepMessage(PcepChannelHandler h, PcepMessage m) throws IOException, PcepParseException {
                ...
                if (m.getType() != PcepType.KEEP_ALIVE) {
                    ...
                    h.pc = h.controller.getPcepClientInstance(h.thispceId, h.sessionId, h.pcepVersion,
                            h.pcepPacketStats);
                    ...
                    if (!h.pc.connectClient()) {
                        ...
                        log.info("Session Established with IP: " + h.peerAddr);
        ...
        void syncLspDb(PcepClientDriver pc) {
            ...
            List<PcePathReport> llPceInitLspList = PcepConfig.getInstance().getPceService()
                    .queryAllInitiateTunnelsByMdsc();
        ...
        protected void disconnectDuplicate(PcepChannelHandler h) {
            ...
            h.duplicatePceIdFound = Boolean.TRUE;
    ...
    public void channelConnected(ChannelHandlerContext ctx, ChannelStateEvent e) throws Exception {
        ...
        log.info("PCEP connected from {}", channel.getRemoteAddress());
        ...
        if (!pcepConfig.isPeerConfigured(peerAddr)) {
            log.debug("Peer is not configured {}", peerAddr);
            channel.close();
            return;
        }

        log.info("PCEP handle connection response from peer");

        sendHandshakeOpenMessage();
        setState(ChannelState.OPENSENT);
        pcepConfig.setPeerConnState(peerAddr, PcepPeerCfg.State.OPENSENT);
    ...
    public void channelDisconnected(ChannelHandlerContext ctx, ChannelStateEvent e) throws Exception {
        ...
        if (thispceId != null) {
            if (!duplicatePceIdFound) {
                ...
                log.info("{}:removal called", getClientInfoString());
                if (pc != null) {
                    pc.removeConnectedClient();
                }

                // Retry connection if connection is lost to bgp speaker/peer
                PcepPeerCfg pcepeer = pcepConfig.getPeer(peerAddr);
                PcepPeerCfg.State peerCfgState = pcepConfig.getPeerConnState(peerAddr);
                if ((channel != null) && (null != channel.getPipeline().get("handler"))
                        && (!peerCfgState.equals(PcepPeerCfg.State.INVALID))) {

                    // on session disconnect using configuration, do not retry
                    if (!peerCfgState.equals(PcepPeerCfg.State.IDLE)) {
                        log.info("Connection reset by peer, retry, STATE:{}", peerCfgState);

                        pcepConfig.setPeerConnState(peerAddr, PcepPeerCfg.State.IDLE);
                        pcepeer.connectPeer().connectPeer();
                    }
                } else {
                    pcepConfig.setPeerConnState(peerAddr, PcepPeerCfg.State.IDLE);
                }
                ...
                log.info("{}:duplicate found", getClientInfoString());
                duplicatePceIdFound = Boolean.FALSE;
            }
        } else {
            pcepConfig.setPeerConnState(peerAddr, PcepPeerCfg.State.IDLE);
    ...
    public void exceptionCaught(ChannelHandlerContext ctx, ExceptionEvent e) throws Exception {
        ...
        if (e.getCause() instanceof ReadTimeoutException) {
            if (ChannelState.OPENSENT == state) {
                ...
                log.info("Sending PCEP-ERROR message to PCC.");
            ...
            log.info("Channel for pc {} already closed", getClientInfoString());
    ...
    public void channelIdle(ChannelHandlerContext ctx, IdleStateEvent e) throws Exception {
        ...
        if (e.getState() == IdleState.READER_IDLE) {
            ...
            log.info("Sending keep alive message due to IdleState timeout " + pc.toString());
    ...
    private void sendHandshakeOpenMessage() throws IOException, PcepParseException {
        ...
        PcepConfig pccConfig = PcepConfig.getInstance();
        if (pccConfig == null) {
            log.error("Failed to get PcepConfig");
            return;
        }
        ...
        PcepOpenObject.Builder openBldr =  factory1.buildOpenObject()
                                            .setSessionId(pcepPeerCfg.getSessionId())
                                            .setKeepAliveTime((byte) PcepPipelineFactory.DEFAULT_KEEP_ALIVE_TIME)
                                            .setDeadTime((byte) PcepPipelineFactory.DEFAULT_DEAD_TIME);

        if (llOptionalTlv != null) {
            openBldr.setOptionalTlv(llOptionalTlv);
        }

        PcepMessage msg = factory1.buildOpenMsg()
                            .setPcepOpenObj(openBldr.build())
                            .build();
