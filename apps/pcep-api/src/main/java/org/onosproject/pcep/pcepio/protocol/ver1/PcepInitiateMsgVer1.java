...
class PcepInitiateMsgVer1 implements PcepInitiateMsg {
    ...
    static class Reader implements PcepMessageReader<PcepInitiateMsg> {
        ...
        public PcepInitiateMsg readFrom(ChannelBuffer cb) throws PcepParseException {
            ...
            log.info("reading PcInitiate message of length " + length);
        ...
        public boolean parsePcInitiatedLspRequestList(ChannelBuffer cb) throws PcepParseException {
            ...
            log.info("PcInitiate message decode begin");
            ...
            while (0 < cb.readableBytes()) {
                ...
                log.info("Parsing Srp Object, Srpid:" + srpObj.getSrpID()
                                 + " R Flag:" + isDelLspRequest);
                ...
                log.info("Parsing Lsp Object, PlspId: " + lspObj.getPlspId());
                ...
                if (!isDelLspRequest) {
                    ...
                    log.info("Parsing End point object SrcIp: " + Ip4Address.valueOf(endPointObj.getSourceIpAddress())
                                     + ", DstIp: " + Ip4Address.valueOf(endPointObj.getDestIpAddress()));
                    ...
                    if (cb.readableBytes() > MINIMUM_COMMON_HEADER_LENGTH) {
                        log.info("decoding attribute list ");
                        pceInitLspReq.setPcepAttribute(PcepAttributeVer1.read(cb));
                    }
                } else {
                    // vn association may come in pcinit delete case
                    if (cb.readableBytes() > MINIMUM_COMMON_HEADER_LENGTH) {
                        cb.markReaderIndex();
                        PcepObjectHeader tempObjHeader = PcepObjectHeader
                                .read(cb);
            ...
            log.info("PcInitiate message decoding finished");
        ...
        public void write(ChannelBuffer cb, PcepInitiateMsgVer1 message) throws PcepParseException {
            ...
            while (listIterator.hasNext()) {
                ...
                if (!isDelLspRequest) {
                    ...
                    LinkedList<PcepAssociationObject> llAssociationObj = listReq.getAssociationObjectList();
