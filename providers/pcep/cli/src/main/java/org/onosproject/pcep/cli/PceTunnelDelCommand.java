 ...
 */
@Command(scope = "onos", name = "pcep-tunnel-del", description = "Test CLI : Supports deleting pce init tunnel.")
public class PceTunnelDelCommand extends AbstractShellCommand {
    ...
    protected void execute() {
        log.info("Execute PCInit Tunnel Dele --- Test");
    ...
    private void pcepSetupTunnelTest(String src, String dst, String symbPath) {
        try {
            ...
            if (null == pc) {
                log.info("Session not exist with ingress -- PCInit Delete");
                return;
            }
            ...
            //build LSP object
            PcepLspObject lspobj = pc.factory().buildLspObject().setAFlag(true).setOFlag((byte) 0).setPlspId(1)
                    .setOptionalTlv(llOptionalTlv).build();
